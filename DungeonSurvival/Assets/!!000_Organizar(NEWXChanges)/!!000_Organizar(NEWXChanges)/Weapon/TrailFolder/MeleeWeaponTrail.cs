...
public class MeleeWeaponTrail : MonoBehaviour
{
	...
	void Update ( )
	{
		...
		if (pointsToUse.Count > 1)
		{
			...
			UnityEngine.Color[] newColors = new UnityEngine.Color[pointsToUse.Count * 2];
			...
			for (int n = 0; n < pointsToUse.Count; ++n)
			{
				...
				UnityEngine.Color color = UnityEngine.Color.Lerp(UnityEngine.Color.white, UnityEngine.Color.clear, time);
				if (_colors != null && _colors.Length > 0)
				{
					...
					color = UnityEngine.Color.Lerp(_colors[(int)min], _colors[(int)max], lerp);
				}
	...
	void OnDrawGizmos ( )
	{
		Gizmos.color = UnityEngine.Color.red; // Puedes cambiar el color del gizmo si lo deseas
		Gizmos.DrawWireSphere(_base.position, 0.1f); // Dibuja una esfera en la posición base
		Gizmos.color = UnityEngine.Color.blue;
		Handles.DrawAAPolyLine(15.0f, new Vector3[] { _base.position, _tip.position }); // Dibuja una línea desde la posición base hasta la posición tip    }
	}
}